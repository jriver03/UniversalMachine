...
static inline void push_id(Stack* s, uint32_t id)
{
    if (s->top == s->cap) {
        size_t nc = s->cap ? s->cap * 2 : 32;
        uint32_t *p = (uint32_t*)realloc(s->buf, nc * sizeof *p);
        #if DEBUG
...
static inline uint32_t pop_id(Stack* s)
{
...
void malloc_array(uint32_t id, unsigned long size)
{
    ensure_capacity(id);
    array_table[id].alloced_array = (uint32_t*) calloc(size, sizeof(uint32_t));
    array_table[id].array_len = size;
}

/* Returns non zero on failure */
int dealloc_array(uint32_t id) {
    #if DEBUG
    if (id == 0) return 1;
...
{
    uint32_t *dst = array_table[0].alloced_array;
    uint32_t x;

    size_t n = fread(dst, sizeof(uint32_t), program_words, program_fp);
...
void reload_program(uint32_t* new_array, unsigned long length)
{
    unsigned long byte_size = length * sizeof(uint32_t);
    uint32_t* temp;
    ...
    if(array_table[0].array_len < length) 
    {
        ...
        array_table[0].alloced_array = (uint32_t*) malloc(byte_size);
    }
...
int execute(uint32_t* program)
{
    int opcode;
    int a, b, c;  /* Register indexes */
    uint32_t* word_array;
    uint32_t program_instr;
    uint32_t input;
    uint32_t alloc_size;
    uint32_t* R = reg.registers; /* Local copy to avoid mem accesses */
    uint32_t pc = reg.r.pc; /* Local copy of frequently used PC */
    ...
    #define FETCH()   do { program_instr = program[pc]; opcode = (program_instr >> 28); } while (0)
    #define DISPATCH() do { goto *jump_table[ /*(opcode <= 13) ? opcode : 14*/ opcode];} while (0)
    #define NEXT()    do { pc++; FETCH(); DISPATCH(); } while (0)
...
op2: /* Array Update */
{
    ...
    if (R[b] >= array_table[R[a]].array_len)
    {
        printf("On line %u: Index %u out of bounds for array length %lu\n",
               pc+1, R[b], array_table[R[a]].array_len);
